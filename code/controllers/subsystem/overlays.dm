...
/atom/proc/cut_overlays(external = FALSE)
	LAZYINITLIST(external_overlays)
	...
	if(external)
		external_overlays.Cut()
	else
		remove_overlays -= external_overlays

	//If not already queued for work and there are overlays to remove
	if(NOT_QUEUED_ALREADY && remove_overlays.len)
		QUEUE_FOR_COMPILE

/atom/proc/cut_overlay(list/overlays, external = FALSE)
	if(!overlays)
		return
	...
	if(external)
		var/list/cached_external = external_overlays
		LAZYREMOVE(cached_external, overlays)
	else
		overlays -= external_overlays
...
/obj/item/station_charter/proc/test_decal()
	var/cube = icon('icons/effects/blood.dmi', "splatter6")
	add_overlay(cube,TRUE)

/obj/item/station_charter/proc/test_decal2()
	var/cube = icon('icons/effects/beam.dmi', "bsa_beam")
	add_overlay(cube)

/atom/proc/add_overlay(list/overlays, external = FALSE)
	if(!overlays)
		return
